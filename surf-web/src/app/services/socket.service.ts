...
export class SocketService {
  private readonly CHUNK_SIZE = 1024 * 10; // 分片大小，10KB
  // 这是主连接的 Socket
  private socket: any
  // 存放指定command的订阅
  // key是command
  ...
  // 分片发送消息
  public send(path: string, command: string, data: any = {}, is_high_priority: boolean = false) {
  if (!this.socket || this.socket.readyState !== WebSocket.OPEN) {
    throw new Error('WebSocket is not connected');
  }

  const request_id = crypto.randomUUID();  // 生成唯一的 request_id
  const message = {
    path,
    command,
    request_id,
...
public request(path: string, command: string, data: any = {}, is_high_priority: boolean = false): Promise<any> {
